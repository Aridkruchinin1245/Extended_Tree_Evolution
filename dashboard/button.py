...
class Button(TextArea):
    def __init__(self, screen: pygame.Surface, color: tuple[int, int, int], rect,
                text, text_color, font_size):
        super().__init__(screen, color, rect, text, text_color, font_size)
        self._unpressed_font_color = text_color
        self._pressed_font_color = tuple(map(lambda a: a // 2, text_color)) #делает цвет темнее
        self._pressed = False
...
class Switch(TextArea):
    ...
    def press(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            mouse_pos = pygame.mouse.get_pos()
            if event.button == 1 and self.rect.left < mouse_pos[0] < self.rect.right and self.rect.top < mouse_pos[1] < self.rect.bottom:
                if not self.pressed:
                    self.color = self._pressed_color
                    self.text_color = self._pressed_font_color
                    self.pressed = True
